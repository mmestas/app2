app.controller('finalReviewerDashCtrl', function ($rootScope, $scope, $state, $stateParams, $location, apiSrvc, NgTableParams, fileUpload, $http, $timeout, $interval, $compile, Upload, upload, $filter, blockUI, ModalService, authSrvc) {
    ...
    $scope.getDashboardData = function () {
        apiSrvc.getData($rootScope.apiUrl + '/Default.aspx?remoteCall=urlSiteGetPatientsStatusNotComplete').then(function (response) {
            ...
            angular.forEach(caseData, function(caseToBeFiltered) {
              caseToBeFiltered.setPatientFinalReviewerDate = $filter('jsonDate')(caseToBeFiltered.setPatientFinalReviewerDate);
        ...
        $scope.byFormattedDateFilter = function(datarow) {
         ...
         // NOTE: This will get changed to Final Reviewer Date
        $scope.assignedFormattedDateFilter = function(datarow) {
          var returnStatus = true;
              var simpleDate = new Date(datarow.setPatientFinalReviewerDate);
              simpleDate = $filter('date')(simpleDate);
              datarow.setPatientFinalReviewerDate = simpleDate;
              return returnStatus;
        }
